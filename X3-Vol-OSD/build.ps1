...
#   X3VolOsd-<version>-setup.exe     installer (per-user, adds Start Menu + uninstaller)
...
[xml]$proj = Get-Content "$root\X3VolOsd\X3VolOsd.csproj"
$version = ($proj.Project.PropertyGroup.Version | Where-Object { $_ }) | Select-Object -First 1
if (-not $version) { throw "No <Version> found in X3VolOsd.csproj" }
...
dotnet test "$root\X3VolOsd.Tests" -c Release
if ($LASTEXITCODE -ne 0) { throw "tests failed - not publishing" }

dotnet publish "$root\X3VolOsd" -c Release -r win-x64 --self-contained false `
    -p:PublishSingleFile=true -p:DebugType=none -o "$root\publish"
if ($LASTEXITCODE -ne 0) { throw "publish failed" }

Copy-Item "$root\publish\X3VolOsd.exe" "$root\dist\X3VolOsd-$version-portable.exe"

if (-not (Test-Path $iscc)) { throw "Inno Setup not found at $iscc" }
& $iscc "/DAppVersion=$version" "$root\installer\X3VolOsd.iss" | Out-Null
